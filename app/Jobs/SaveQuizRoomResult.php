...
class SaveQuizRoomResult implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        {
            if ($attempt->user_id != $respreformance['user_id']) 
            {
                $attempt = Attempt::where('parent_id', $respreformance['quiz_id'])->where('user_id', $respreformance['user_id'])->first();
